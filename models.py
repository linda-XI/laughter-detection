...
from model_utils import MBConvConfig, Conv2dNormActivation, BasicBlock, Bottleneck, conv1x1, DW, BasicBlock_DW
...
class ResNet8_baseline(nn.Module):
    def __init__(self, num_classes=1, dropout_rate=0.5, linear_layer_size=960, filter_sizes=[32, 16, 16, 16]):
        super(ResNet8_baseline, self).__init__()
        print(f"training with dropout={dropout_rate}")
        # Initial input conv
        self.conv1 = nn.Conv2d(
            in_channels=1, out_channels=32, kernel_size=(3, 3),
...
class ResNet10(nn.Module):
    def __init__(
        ...
        self.layer2 = self._make_layer(block, filter_sizes[1], layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, filter_sizes[2], layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, filter_sizes[3], layers[3], stride=2,
...
class ResNet8_DW(nn.Module):
    def __init__(self, num_classes=1, dropout_rate=0.5, linear_layer_size=960, filter_sizes=[32, 16, 16, 16]):
        super(ResNet8_DW, self).__init__()
        print(f"training with dropout={dropout_rate}")
        # Initial input conv
        self.conv1 = nn.Conv2d(
            in_channels=1, out_channels=32, kernel_size=(3, 3),
...
class ResNet18_DW(nn.Module):
    def __init__(
            self,
            block: Type[Union[BasicBlock, Bottleneck, BasicBlock_DW]] = BasicBlock_DW,
            layers: List[int] = [2, 2, 2, 2],
            num_classes: int = 1,
            zero_init_residual: bool = False,
            groups: int = 1,
        ...
        super(ResNet18_DW, self).__init__()
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck) and m.bn3.weight is not None:
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlock_DW) and m.bn2.weight is not None:
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]

    def _make_layer(
            self,
            block: Type[Union[BasicBlock, Bottleneck, BasicBlock_DW]],
            planes: int,
            blocks: int,
            stride: int = 1,
            dilate: bool = False,
